...
public class Main extends SimpleApplication {
    ...
    private String[] aiLevels = {"Nul", "Mauvais", "Medium", "Bon", "Difficile", "Expert", "Impossible"};
    ...
    Demarre startDemarre = new Demarre();
    ...
    public void simpleInitApp() {
        ...
        stateManager.attach(startDemarre);
        // this.paused = true;
        ...
        inputManager.addMapping("AugmenteIA", new KeyTrigger(KeyInput.KEY_U));
        inputManager.addMapping("BaisseIA", new KeyTrigger(KeyInput.KEY_D));
        inputManager.addListener(actionListener, "AugmenteIA", "BaisseIA");

        cam.setLocation(new Vector3f(10, 15*scale, 27*scale));
    ...
    private ActionListener actionListener = new ActionListener() {
        public void onAction(String name, boolean keyPressed, float tpf) {
            if (name.equals("AugmenteIA") && !keyPressed) {
                if(aiIniSpeed<8)
                    aiIniSpeed+=1;
            }
            if (name.equals("BaisseIA") && !keyPressed) {
                if(aiIniSpeed>2)
                    aiIniSpeed-=1;
            }
        }
    ...
    public void initScoreDisplay(){
        ...
        playerScoreDisplay.setSize(guiFont.getCharSet().getRenderedSize());
        playerScoreDisplay.setColor(ColorRGBA.White);
        playerScoreDisplay.setText("Player Score : 0");
        playerScoreDisplay.setLocalTranslation(9*scale, 8*scale, -4*scale);

        aiScoreDisplay.setSize(guiFont.getCharSet().getRenderedSize());
        aiScoreDisplay.setColor(ColorRGBA.White);
        aiScoreDisplay.setText("Computer Score: 0");
        aiScoreDisplay.setLocalTranslation(-9*scale, 8*scale, -4*scale);

        aiLevelDisplay.setSize(guiFont.getCharSet().getRenderedSize());
        aiLevelDisplay.setColor(ColorRGBA.White);
        aiLevelDisplay.setText("AI Difficulty :");
        aiLevelDisplay.setLocalTranslation(0, 8*scale, -4*scale);
    ...
    public void initTable(){
        ...
        //tableFrontGeom1.setLocalTranslation(-10.0f * scale, 1 * scale, 14 * scale);
        tableSideGeom1.setLocalTranslation(9f*scale, 1*scale, 0*scale);
        tableSideGeom2.setLocalTranslation(-9f*scale, 1*scale, 0*scale);
        tableFrontGeom1.setLocalTranslation(4.0f*scale, 1*scale, 14*scale);
        tableFrontGeom2.setLocalTranslation(-4.0f*scale, 1*scale, 14*scale);
        tableBackGeom1.setLocalTranslation(4.0f*scale, 1*scale, -14*scale);
        tableBackGeom2.setLocalTranslation(-4.0f*scale, 1*scale, -14*scale);
    ...
    public void initMaterials(){
        ...
        tableSideMat.setColor("Color", ColorRGBA.Brown);
        tableFrontAndBackMat.setColor("Color", ColorRGBA.White);
...
/*
* Setting up goal width, and scoring, placing the puck again
* */
        //puckX<4f*scale && puckX>-10f
        //puckX<3f*scale && puckX>-10f
        if(puckZ<-12f*scale && Math.abs(puckX) < 10f*scale){
            playerScore++;
        ...
        aiScoreDisplay.setText("Jouer: "+playerScore);
        playerScoreDisplay.setText("IA: "+ aiScore);
        aiLevelDisplay.setText("Difficulte IA : "+aiLevels[(int)aiIniSpeed-2]);

        if(playerScore >=12){
            stateManager.detach(bulletAppState);
            cheerSound.play();
            playerScoreDisplay.setText("T'as gagne!");
            aiScoreDisplay.setText("Felicitations!");
        ...
        if(aiScore >=12){
            stateManager.detach(bulletAppState);
            booSound.play();
            playerScoreDisplay.setText("T'as PERDU!");
            aiScoreDisplay.setText("Pratiques!");
        }
